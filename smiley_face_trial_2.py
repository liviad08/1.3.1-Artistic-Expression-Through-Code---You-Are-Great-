...
def run_animation():
    """Triggers when Space is pressed. Manages logic for the message position."""
    global press_count
    press_count += 1
    artist.clear()
    ...
    # 2. Conditional Logic for Smiley and Message Position
    if press_count >= 5:
        # Smiley appears and grows
        growth_size = 50 + (press_count * 5)
        draw_smiley(growth_size)
        
        # Message moves to the bottom to avoid being hidden
        text_y_position = -300
    else:
        # Message stays in the middle
        text_y_position = -20

    # 3. Write the message (String Manipulation + Changing Colors)
    artist.penup()
